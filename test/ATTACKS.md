...
3. Gates, all of which must hold before a rule lands: `npm test` passes, the chat false-positive sweep in `detector.test.js` gains no new hits, `npm run eval:data` shows no new false positives on the benign records, and `PAIRS=300 npm run eval:data` (every benign record against 300 past answers, and every sentence of `foreign_chat.json` against every past answer; needs `ml/data/answers.txt` from `ml/scripts/fetch_answers.py`) stays near its baseline. Round 1 baseline: 175 of 73,440 pairs before, 179 after. A rule that adds more than a handful of pairs is not worth it; the sounds-like-across-tokens rule added 15 and was dropped.
